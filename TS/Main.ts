...
import { falaFigurante, falaPersonagem, stop } from "./Auxiliares/Auxiliares";
...
async function jogo(): Promise<void> {

    await falaPersonagem(`Salve, aqui é a juliana!`);

    await falaFigurante(`Fala, eu sou o figurante cara!`);

    await falaPersonagem(`O que está acontecendo aqui?`);
